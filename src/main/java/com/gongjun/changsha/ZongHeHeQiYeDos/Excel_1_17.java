...
/**
 * @Description:处理表格922-1.xls中Sheet:"1-17",对应关系:1-17V （清查平台）
 * @Author: GongJun
 * @Date: Created in 10:37 2020/10/13
 */
public class Excel_1_17 {
    /**
     * @修改点******************************************************************
     */
    ...
    public static void todo() {
        //按地区分组
        List<List<Object>> dataSheetDatas = excelDos("1-17", Arrays.asList(0), new ArrayList(), 0);
        //按登记类型分组
    ...
    public static List<List<Object>> excelDos(String keywords, List exceptRows, List exceptCols, int mark) {
        ...
        for (File f : fs) {                //遍历File[]数组
            if (!f.isDirectory())        //若非目录(即文件)，则打印
            {
                 ...
                 * @修改点******************************************************************
                 * */
                if (fileName != null && StringUtils.startsWithIgnoreCase(fileName, keywords) && StringUtils.endsWithIgnoreCase(fileName, ".xls"))
...
//        List exceptCols = Arrays.asList(1);
        ...
        if (mark == 0) dataSheetDatas.add(1, Arrays.asList("按地区分组", null, null, null, null));
        if (mark == 0) dataSheetDatas.add(农林牧渔业Mark+1, Arrays.asList("按行业(门类)分组", null, null, null, null));
        return dataSheetDatas;
    }
    ...
    public static void dataWriteDos(List<List<Object>> dataSheetDatas) {
         ...
         * @修改点******************************************************************
         * */
        Sheet standardSheet = standardWorkbook.getSheet("1-17");
