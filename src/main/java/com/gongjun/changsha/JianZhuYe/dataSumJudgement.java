...
/**
 * @Description: 判断区县数据之和是否等于全市数据(贸易)
 * @Author: GongJun
 * @Date: Created in 9:24 2020/10/21
 */
public class dataSumJudgement {
    //全市数据表格路径
    private static String upLevel = "D:\\长沙项目\\建筑业\\全市";

    //区县数据表格路径
    private static String downLevel = "D:\\长沙项目\\建筑业\\已处理";
    ...
    public static void dataJudge() {
        ...
        for (File fileOne : upExcelList) {
            ...
            for (int i = 1; i < sheetNumber; i++) {
                ...
                if(!upSheet.getSheetName().equals("7-04")) continue;
                for (int j = 0; j < upSheet.getPhysicalNumberOfRows(); j++) {
                    ...
                    for (int k = 0; k < upRow.getPhysicalNumberOfCells(); k++) {
                        ...
                        if (Math.abs((int) valueOne - valueTwoInt) > 5) {
                            System.out.println("[" + fileName + ":" + upSheet.getSheetName() + "]--" + RegUtils.delAllSpaceForString(upRow.getCell(0).getStringCellValue()) +
                                    "," + "第" + (k + 1) +
                                    "列数据不一致,地方总和:" + valueTwoInt + ",全市数据:" + valueOne
                                    +",差值[地方和-市表]:"+(valueTwoInt - (int)valueOne)
                            );
                        }
                    }
                }
