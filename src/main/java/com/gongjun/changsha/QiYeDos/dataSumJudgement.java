package com.gongjun.changsha.QiYeDos;
...
//    private static String downLevel = "D:\\长沙项目\\科技\\地区";
    ...
    public static void dataJudge() {
        //获取全市的表格
        List<File> upExcelList = com.gongjun.changsha.FuWuYeDos.dataSumJudgement.readFileList(upLevel);
        if (CollectionUtils.isEmpty(upExcelList)) return;
        for (int i = 0; i < upExcelList.size(); i++) {
            if (upExcelList.get(i).getAbsolutePath().contains("高新") || upExcelList.get(i).getAbsolutePath().contains("922-1")) upExcelList.remove(i);
        }
        if (CollectionUtils.isEmpty(upExcelList)) return;
        //获取区县表格
        List<File> downExelList = com.gongjun.changsha.FuWuYeDos.dataSumJudgement.readFileList(downLevel);

        if (CollectionUtils.isEmpty(downExelList)) return;
        for (int i = 0; i < downExelList.size(); i++) {
            if (downExelList.get(i).getAbsolutePath().contains("高新") || downExelList.get(i).getAbsolutePath().contains("922-1")) downExelList.remove(i);
        }
