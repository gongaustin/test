...
public class Excel_10_02 {
    ...
    public static List<List<Object>> readData(String sourceExcelPath) {
        ...
        List<Integer> sourceCols = Arrays.asList(0, 2, 3, 4);
        int neiziMark = 0;
        for (int i = 0; i < sourceSheet.getPhysicalNumberOfRows(); i++) {
            ...
            for (int j = 0; j < sourceCols.size(); j++) {
                ...
                if (j == 0) {
                    ...
                    if("内资企业".equals(RegUtils.delAllSpaceForString(index))) neiziMark = i - 7;
        ...
        if(neiziMark != 0) data.add(neiziMark,Arrays.asList("按登记注册类型分组",null,null,null));
        return data;
    }
    ...
    public static void writeData(String targetExcelPath, List<List<Object>> datas) {
         ...
         * @修改点******************************************************************
         * */
        Sheet targetSheet = targetWorkbook.getSheet("10-02");
    ...
    @Test
    public void test(){
        todo("D:\\长沙项目\\房地产\\地区\\1-2\\芙蓉区\\(fdc-4301-02)按地区、登记注册类型分全部房地产开发企业资产总计、负债合计2020-11-18.xlsx",
